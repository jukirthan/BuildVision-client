...
The AI Assistant calls the authenticated `/api/ai/chat` endpoint. It supports
OpenAI and Google Gemini through a provider switch in the chat header. Configure
`OPENAI_API_KEY`/`OPENAI_MODEL` and/or `GEMINI_API_KEY`/`GEMINI_MODEL` on the
Flask/Railway/AWS backend only. `AI_PROVIDER=gemini` selects Gemini by default.
Do not add either key to `NEXT_PUBLIC_*` variables or expose them in the browser.
